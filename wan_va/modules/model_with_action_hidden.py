...
class WanTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def get_action_hidden_states(self, actions):
        """
        Get the hidden state representation of actions.
        This is useful for extracting action embeddings that can be concatenated
        or manipulated in latent space before being fed to the transformer.

        Args:
            actions (torch.Tensor): Action tensor with shape [B, C, F, H, W] or [B, C, F, H, 1]

        Returns:
            torch.Tensor: Action hidden states with shape [B, L, C] where L = F * H * W
        """
        ...
        if actions.dim() == 4:
            ...
            raise ValueError(f"Actions must have 4 or 5 dimensions, got {actions.dim()}")

        # Rearrange and embed
        hidden_states = rearrange(actions, 'b c f h w -> b (f h w) c')
        hidden_states = self.action_embedder(hidden_states)

        return hidden_states

    def concat_action_hidden_states(self, action_hidden_list):
        """
        Concatenate multiple action hidden states.

        Args:
            action_hidden_list (List[torch.Tensor]): List of action hidden states,
                each with shape [B, L_i, C]

        Returns:
            torch.Tensor: Concatenated action hidden states with shape [B, sum(L_i), C]
        """
        return torch.cat(action_hidden_list, dim=1)
    ...
    def forward(
        ...
        Args:
            ...
            action_hidden_states (Tensor, *optional*):
                Pre-computed action hidden states in shape [B, L, C].
                If provided, skips action_embedder and uses these directly.
                Useful for concatenating action hidden states from different sources.
        ...
        if action_mode:  # action input emb
            # Use provided action hidden states if available, otherwise compute from actions
            if action_hidden_states is not None:
                # Directly use the provided action hidden states
                latent_hidden_states = action_hidden_states
            else:
                # Original logic: embed actions from noisy latents
